...
export default function StudentsPage() {
  ...
  const [statusFilter, setStatusFilter] = useState<'All' | 'Activated' | 'Not Activated' | 'Working'>('All');
      ...
      <div className="space-y-8 max-w-7xl mx-auto">
        ...
        <div className="bg-white dark:bg-zinc-900 p-6 rounded-3xl border border-zinc-200 dark:border-zinc-800 shadow-sm space-y-4">
          ...
          <div className="flex items-center gap-2 pt-2 border-t border-zinc-100 dark:border-zinc-800 text-xs">
            <span className="font-semibold text-zinc-500">Status filter:</span>
            {(['All', 'Activated', 'Not Activated', 'Working'] as const).map((st) => (
              <button
                key={st}
                  ...
                  {filteredStudents.map((st) => (
                    <tr key={st.id} className="hover:bg-zinc-50/80 dark:hover:bg-zinc-800/40 transition-colors group">
                      ...
                      <td className="px-6 py-4">
                        ...
                        >
                          {st.status === 'Activated' || st.status === 'Working' ? (
                            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-emerald-50 dark:bg-emerald-950/60 border border-emerald-200 dark:border-emerald-800 text-emerald-700 dark:text-emerald-300 font-semibold text-[10px]">
                              Activated
                            </span>
                          ) : (
                            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-amber-50 dark:bg-amber-950/60 border border-amber-200 dark:border-amber-800 text-amber-700 dark:text-amber-300 font-semibold text-[10px]">
                              Not Activated
                            </span>
                          )}
                        </button>
